#include "PauseScreen.h"

PauseScreen::PauseScreen() 
{
    m_screen.setTexture(*(Singleton::instance().getScreen(PAUSE_m)));

    m_font.loadFromFile("arial.ttf");

    sf::Text titleText;
    titleText.setFont(m_font);
    titleText.setString("PAUSED");
    titleText.setCharacterSize(36);
    titleText.setFillColor(sf::Color::Black);
    titleText.setPosition(static_cast<float>(m_titleRect.left + 20.0), static_cast<float>(m_titleRect.top + 15.0));

    sf::RectangleShape titleRect;
    titleRect.setSize(sf::Vector2f(static_cast<float>(m_titleRect.width), static_cast<float>(m_titleRect.height)));
    titleRect.setPosition(static_cast<float>(m_titleRect.left), static_cast<float>(m_titleRect.top));
    titleRect.setFillColor(sf::Color::Black);


    m_PauseScreenTexts.push_back(titleText);
    m_Rectangles.push_back(titleRect);

    std::vector<std::pair<sf::IntRect*, std::string>> buttons = {
        {&m_continue, "CONTINUE"},
        //{&m_settings, "RESTART"},
        {&m_settings, "SETTINGS"},
        {&m_settings, "MENU"}

    };

    for (int i = 0 ; i < buttons.size() ; i++)
    {
        sf::Text text;
        text.setFont(m_font);
        text.setString(buttons.at(i).second);
        text.setCharacterSize(24);
        text.setFillColor(sf::Color::White);
        text.setPosition(320 , 200 + 120 * i);

        sf::RectangleShape rect;
        rect.setSize(sf::Vector2f(200, 50));
        rect.setPosition(300, 100 + 120 * i);
        rect.setFillColor(sf::Color(100, 100, 100, 200));

        m_PauseScreenTexts.push_back(text);
        m_Rectangles.push_back(rect);
    }
}

Screens_m PauseScreen::handleEvents(sf::RenderWindow& window) {
    sf::Event event;
    while (window.pollEvent(event)) {
        switch (event.type) {
        case sf::Event::Closed:
            window.close();
            break;
        case sf::Event::MouseButtonReleased:
            if (event.mouseButton.button == sf::Mouse::Left) {
                sf::Vector2i mousePos(event.mouseButton.x, event.mouseButton.y);

                for (size_t i = 0; i < m_Rectangles.size(); ++i) {
                    if (m_Rectangles[i].getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
                        Singleton::instance().getSoundManager().playSound("click");
                        switch (i) {
                        case 1:
                            return GAME_m;
                        //case 1:
                        //    return GAME_m;
                        case 2:
                            return SETTINGS_m;
                        case 3:
                            return MENU_m;
                        default:
                            break;
                        }
                    }
                }
            }
            break;
        case sf::Event::MouseMoved:
            updateWavingText(sf::Vector2i(event.mouseMove.x, event.mouseMove.y));
            break;
        case sf::Event::Resized:
            std::cout << "yessssssss!";
            break;
        default:
            break;
        }
    }
    return PAUSE_m;
}



void PauseScreen::updateWavingText(sf::Vector2i mousePos)
{
    static const float waveAmplitude = 5.0f;
    static const float waveFrequency = 10.0f;

    for (size_t i = 1; i < m_PauseScreenTexts.size(); ++i)
    {
        if (m_Rectangles[i].getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePos)))
        {
            float time = m_waveClock.getElapsedTime().asSeconds();
            float waveOffset = waveAmplitude * std::sin(time * waveFrequency);
            m_PauseScreenTexts[i].setPosition(static_cast<float>(m_Rectangles[i].getPosition().x) + 20.0f, static_cast<float>(m_Rectangles[i].getPosition().y) + 10.0f + waveOffset);
        }
        else {
            m_PauseScreenTexts[i].setPosition(static_cast<float>(m_Rectangles[i].getPosition().x) + 20.0f, static_cast<float>(m_Rectangles[i].getPosition().y) + 10.0f);
        }
    }
}

void PauseScreen::render(sf::RenderWindow& window)
{
    window.draw(m_screen);
    for (size_t i = 0; i < m_Rectangles.size(); ++i)
    {
        window.draw(m_Rectangles[i]);
        window.draw(m_PauseScreenTexts[i]);
    }
}
//
//
//Menu_p PauseScreen::getMenuButton(sf::Vector2i mousePos)
//{
//    if (m_continue.contains(mousePos)) {
//        return CONTINUE_p;
//    }
//    else if (m_settings.contains(mousePos)) {
//        return SETTINGS_p;}
//    
//    return Menu_p();
//}



//
//#include "PauseScreenScreen.h"
//
//PauseScreenScreen::PauseScreenScreen() {
//    if (!m_font.loadFromFile("arial.ttf")) {
//        std::cerr << "Couldn't load the font!" << std::endl;
//        std::exit(-1);
//    }
//
//    m_continueText.setFont(m_font);
//    m_continueText.setString("Continue");
//    m_continueText.setCharacterSize(30);
//    m_continueText.setPosition(400, 200);
//
//    m_menuText.setFont(m_font);
//    m_menuText.setString("Menu");
//    m_menuText.setCharacterSize(30);
//    m_menuText.setPosition(400, 300);
//
//    m_soundSettingsText.setFont(m_font);
//    m_soundSettingsText.setString("Sound Settings");
//    m_soundSettingsText.setCharacterSize(30);
//    m_soundSettingsText.setPosition(400, 400);
//}
//
//Screens_m PauseScreenScreen::handleEvents(sf::RenderWindow& window) {
//    sf::Event event;
//    while (window.pollEvent(event)) {
//        if (event.type == sf::Event::Closed) {
//            window.close();
//        }
//
//        if (event.type == sf::Event::MouseButtonPressed) {
//            if (event.mouseButton.button == sf::Mouse::Left) {
//                sf::Vector2f worldPos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
//                if (m_continueText.getGlobalBounds().contains(worldPos)) {
//                    return Screens_m::GAME_m;
//                }
//                else if (m_menuText.getGlobalBounds().contains(worldPos)) {
//                    return Screens_m::MENU_m;
//                }
//                else if (m_soundSettingsText.getGlobalBounds().contains(worldPos)) {
//                    return Screens_m::SETTINGS_m;
//                }
//            }
//        }
//    }
//    return Screens_m::PauseScreen_m;
//}
//
//void PauseScreenScreen::render(sf::RenderWindow& window) {
//    window.clear();
//    window.draw(m_continueText);
//    window.draw(m_menuText);
//    window.draw(m_soundSettingsText);
//    window.display();
//}


//no error but you have to make it print it there is a bug 